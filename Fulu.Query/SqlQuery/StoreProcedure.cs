...
namespace Fulu.Query.SqlQuery
{
    ...
    public sealed class StoreProcedure : IDbExecute
	{
		...
		public StoreProcedure(string spName, params DbParameter[] parameters)
		{
			...
			this.Command = ProviderManager.CreateCommand();
		...
		internal static DbParameter[] GetSpParameters(object parameterObject)
		{
			...
			foreach( PropertyInfo property in properties ) {
				...
				if( value == null || value  == DBNull.Value) {
					...
					para = ProviderManager.CreateParameter(name, DBNull.Value);
					...
					if( spOutParam != null ) {
						...
						if( value == null || value == DBNull.Value ) {
							...
							para = ProviderManager.CreateParameter(name, spOutParam.Value);
						...
						//支持匿名对象中直接使用DbParameter
						DbParameter parameter = value as DbParameter;
						if( parameter != null ) {
							para = parameter;
						}
						else {
							para = ProviderManager.CreateParameter(name, value);
        ...
        {
            return DbHelper.ExecuteReader(this.Command);
        }
